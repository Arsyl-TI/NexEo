        ...
        if (isOpen.value) {
          ...
          if (loading.value) {
            ...
            ssrRenderList(allResults.value, (item, idx) => {
              _push2(`<div class="${ssrRenderClass(["p-3 rounded-2xl border transition-all cursor-pointer flex items-center justify-between gap-3", selectedIndex.value === idx ? "bg-primary/15 border-primary/50 text-foreground scale-[1.01]" : "bg-card/40 border-border/60 text-muted-foreground hover:bg-card"])}" data-v-d0e79b46><div class="flex items-center gap-3 min-w-0" data-v-d0e79b46><div class="w-9 h-9 rounded-xl bg-card border border-border flex items-center justify-center text-lg shrink-0" data-v-d0e79b46>${ssrInterpolate(item.type === "video" ? "🎬" : item.type === "novel" ? "📖" : "📤")}</div><div class="min-w-0" data-v-d0e79b46><h4 class="font-bold text-xs text-foreground truncate" data-v-d0e79b46>${ssrInterpolate(item.title || item.name)}</h4><p class="text-[10px] text-muted-foreground font-mono truncate" data-v-d0e79b46>${ssrInterpolate(item.subtitle || item.category || item.sizeFormatted || item.type)}</p></div></div><div class="flex items-center gap-2 shrink-0" data-v-d0e79b46><span class="text-[10px] uppercase font-bold font-mono px-2 py-0.5 rounded-md bg-background border border-border text-primary" data-v-d0e79b46>${ssrInterpolate(item.type)}</span><span class="text-xs text-muted-foreground" data-v-d0e79b46>↵</span></div></div>`);
            });
            _push2(`<!--]--></div>`);
          }
...
//# sourceMappingURL=default-DHQE3e2r.js.map
