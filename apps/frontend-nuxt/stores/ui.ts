...
export const useUIStore = defineStore('ui', () => {
  ...
  const readerTheme = ref<'dark' | 'light' | 'sepia'>('dark')
  ...
  const initPreferences = () => {
    ...
    try {
      const savedTheme = localStorage.getItem('reader_theme') as 'dark' | 'light' | 'sepia' | null
      if (savedTheme) readerTheme.value = savedTheme

      const savedSize = localStorage.getItem('reader_font_size')
      if (savedSize) readerFontSize.value = parseInt(savedSize, 10)
    } catch (err) {
      console.warn('Failed to load preferences from localStorage:', err)
    }
  }

  const setReaderTheme = (theme: 'dark' | 'light' | 'sepia') => {
  ...
  const themeClasses = computed<ReaderThemeClasses>(() => {
