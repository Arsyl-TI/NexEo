...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const chapters = computed(() => {
      var _a;
      return ((_a = mangaStore.currentManga) == null ? void 0 : _a.chapters) || [];
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if ((_a = manga.value) == null ? void 0 : _a.cover) {
      ...
      if (unref(mangaStore).loading) {
        ...
        if (manga.value.cover) {
          ...
          _push(`<div class="flex flex-col items-center justify-center h-full text-muted-foreground text-xs p-4 text-center" data-v-4c54d9a8><span data-v-4c54d9a8>\u{1F3A8}</span><span class="mt-2" data-v-4c54d9a8>No Cover</span></div>`);
        }
        ...
        if (hasResumeChapter.value) {
          ...
          _push(ssrRenderComponent(_component_NuxtLink, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<span data-v-4c54d9a8${_scopeId}>\u{1F3A8}</span> Mulai Membaca Chapter 1 `);
              } else {
                return [
                  createVNode("span", null, "\u{1F3A8}"),
        ...
        if (manga.value.author) {
          _push(`<div class="inline-flex items-center gap-2 bg-purple-900/30 border border-purple-500/30 text-purple-300 px-3.5 py-1.5 rounded-full font-medium" data-v-4c54d9a8><span data-v-4c54d9a8>\u{1F464}</span> Author: <span class="font-bold text-white" data-v-4c54d9a8>${ssrInterpolate(manga.value.author)}</span></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
        ...
        _push(`<div class="flex flex-col sm:flex-row gap-3 justify-between items-center bg-card/40 border border-border/60 p-3 rounded-2xl mb-4" data-v-4c54d9a8><div class="relative w-full sm:w-72" data-v-4c54d9a8><input${ssrRenderAttr("value", chapterSearch.value)} type="text" placeholder="Cari chapter komik..." class="w-full bg-background border border-border rounded-xl pl-9 pr-4 py-2 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary" data-v-4c54d9a8><span class="absolute left-3 top-2.5 text-xs text-muted-foreground" data-v-4c54d9a8>\u{1F50D}</span></div><button class="px-3 py-2 bg-card border border-border rounded-xl text-xs font-semibold text-muted-foreground hover:text-foreground flex items-center gap-1.5" data-v-4c54d9a8><span data-v-4c54d9a8>${ssrInterpolate(chapterSort.value === "asc" ? "\u2B07 Urut Awal" : "\u2B06 Urut Akhir")}</span></button></div>`);
...
//# sourceMappingURL=index-CrGzZAvu.mjs.map
