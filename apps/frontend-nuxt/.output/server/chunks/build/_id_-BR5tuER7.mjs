    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (loading.value) {
        ...
        if (video.value.author) {
          _push(`<span class="bg-purple-900/30 border border-purple-500/30 text-purple-300 px-3 py-1 rounded-full font-semibold" data-v-3e42edcc> \u{1F464} ${ssrInterpolate(video.value.author)}</span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<span class="bg-primary/20 text-primary border border-primary/30 px-3 py-1 rounded-full font-semibold uppercase" data-v-3e42edcc>${ssrInterpolate(video.value.format)}</span><span class="bg-gray-800/80 border border-gray-700 text-gray-300 px-3 py-1 rounded-full font-medium" data-v-3e42edcc> \u{1F4BE} ${ssrInterpolate(video.value.sizeFormatted)}</span><span class="bg-gray-800/80 border border-gray-700 text-gray-300 px-3 py-1 rounded-full font-medium" data-v-3e42edcc> \u{1F4C1} ${ssrInterpolate(video.value.folder)}</span></div></div><div class="flex flex-wrap gap-2 shrink-0" data-v-3e42edcc><a${ssrRenderAttr("href", `/api/video/${encodeURIComponent(video.value.id)}/stream`)} download class="btn-primary px-4 py-2.5 text-xs font-semibold flex items-center gap-2" data-v-3e42edcc><svg width="18" height="18" viewBox="0 0 24 24" fill="none" data-v-3e42edcc><path d="M12 16L7 11L8.4 9.55L11 12.15V4H13V12.15L15.6 9.55L17 11L12 16ZM6 20C5.45 20 4.979 19.804 4.587 19.412C4.195 19.02 4 18.55 4 18V15H6V18H18V15H20V18C20 18.55 19.804 19.02 19.412 19.412C19.02 19.804 18.55 20 18 20H6Z" fill="currentColor" data-v-3e42edcc></path></svg> Unduh Video </a></div></div><div class="mt-4 pt-4 border-t border-border/50 flex flex-wrap items-center justify-between gap-3" data-v-3e42edcc><div class="flex items-center gap-2" data-v-3e42edcc><span class="text-xs text-muted-foreground font-medium" data-v-3e42edcc>\u26A1 Kecepatan:</span><!--[-->`);
        ...
        if (video.value.description) {
          _push(`<div class="mt-6 pt-5 border-t border-border/50" data-v-3e42edcc><h3 class="text-sm font-bold text-foreground mb-2 flex items-center gap-2" data-v-3e42edcc><span data-v-3e42edcc>\u{1F4C4}</span> Deskripsi Video </h3><div class="text-xs text-muted-foreground leading-relaxed whitespace-pre-line bg-background/50 border border-border/50 p-4 rounded-xl max-h-60 overflow-y-auto font-sans" data-v-3e42edcc>${ssrInterpolate(video.value.description)}</div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div>`);
...
//# sourceMappingURL=_id_-BR5tuER7.mjs.map
